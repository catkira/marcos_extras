...
if [[ "$#" -ne 2 || ($2 != "rp-125" && $2 != "rp-122" ) ]]; then
    ...
    echo "Example usage: "
    echo "   ./copy_bitstream_bm.sh 192.168.1.163 rp-122"
    exit
fi
...
if [[ $rp_uname == "redpitaya" ]]; then
    ...
    scp ocra_mri_${2}_bm.bit.bin root@$1:/lib/firmware/ocra_mri.bit.bin
    scp ocra_mri_${2}_bm.dtbo root@$1:/lib/firmware/
...
EOF
    ...
    scp ocra_mri_${2}_bm.bit root@$1:/tmp/ocra_mri.bit
    ssh root@$1 <<EOF
